...
def process_good_transection(file_path):
    ...
    processor_goods_transection =dprs(file_path, expected_cols_goods_transection)
...
Processor_goods_transection = process_good_transection(file_path_goods_transection)
print(Processor_goods_transection.data)

file_path_clints_data= r"D:\monymovment\Cashflows\Excel_files\SBAccMFDtlRpt.xls"

def processor_Clints(file_path):
    processor_Clints = dprs(file_path, ['rec_id', 'acc_cd', 'acc_nm', 'Text85', 'نص93'])
...
processor_clints_data = processor_Clints(file_path_clints_data)
